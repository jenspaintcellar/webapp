...
export async function POST(request: Request) {
  ...
  const attendees = body.attendees as { first_name?: string; last_name?: string; phone?: string; age?: number; waiver_accepted?: boolean }[];
  if (!attendees.length || attendees.length > 12 || attendees.some((attendee) => !attendee.first_name?.trim() || !attendee.last_name?.trim() || !attendee.phone?.trim() || (attendee.age !== undefined && (attendee.age < 1 || attendee.age > 120)) || attendee.waiver_accepted !== true)) return NextResponse.json({ error: 'Every attendee needs a name, phone number, and waiver acceptance.' }, { status: 400 });
  ...
  const attendeeRows = attendees.map((attendee, index) => ({ booking_id: booking.id, first_name: attendee.first_name!.trim(), last_name: attendee.last_name!.trim(), email: body.email!.trim().toLowerCase(), phone: attendee.phone!.trim(), age: attendee.age, is_primary: index === 0 }));
