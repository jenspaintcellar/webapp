...
export default function RegistrationPage() {
  ...
  async function startPayment() {
    ...
    const response = await fetch('/api/stripe/checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ bookingId, customerId: customer.id, email }) });
  ...
  function validateAttendees() {
    if (attendees.slice(1).some((attendee) => !attendee.first_name.trim() || !attendee.last_name.trim() || !attendee.phone.trim())) { setMessage('Please complete every attendee name and phone number.'); return false; }
